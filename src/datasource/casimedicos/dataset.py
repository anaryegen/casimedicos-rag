...
from model.config import GOAL_ANS_IDX, GOAL_ARG_GEN, PROMPT_01_CLEAN, PROMPT_02_MEDRAG, PROMPT_FEW_SHOT, PROMPT_RAG, PROMPT_ZERO_SHOT
...
def build_prompt(example:dict,
				 ...
				 train_data_path: str = '/gaueko0/users/ayeginbergenov001/MedExpQA-edited/data/train.en.casimedicos.rag.jsonl'):
	...
	# if not max_rag_docs:
		template = PROMPT_TEMPLATES[PROMPT_ZERO_SHOT][lang]
		...
		template = PROMPT_TEMPLATES[PROMPT_RAG][lang]
		question_text = f"{qtype}: {full_question}"
		options_text = "\n".join([f'{i}. {option.strip()}' for i, option in options.items() if isinstance(option, str)])
		answer_text = f"{correct_option}. {options[str(example['correct_option'])].strip()}"
		priority_text = template.format(grounding="", question=question_text, options=options_text, answer=f"{correct_option}.")
		...
		# get $shot examples
		template = PROMPT_TEMPLATES[PROMPT_FEW_SHOT][lang]
		ex = []
		...
		full_text = template.format(fewshot_example='\n'.join(ex), question=question_text, options=options_text, answer=answer_text)
...
def prepare_data(
	...
	if goal == GOAL_ARG_GEN:
		...
		input_text, answer_text, full_text, _ = build_prompt(example, 
															 tokenizer, 
															 prompt_style, 
															 shot,
															 grounding_entities,
