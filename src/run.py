...
def train_seq2seq(
    ...
    #training_datasets_path = load_dataset(data_args.train_files, data_args.grounding_langs, split='train')
    #development_datasets_path = load_dataset(data_args.train_files, data_args.grounding_langs, split='validation')
...
def inference_seq2seq(
    ...
    if training_args.do_train:
        if not checkpoint_path:
            ...
            if model_args.use_lora:
                model_path = model_args.model_name_or_path
                # model_path = custom_args.model_name_or_path
    ...
    if model_args.use_lora and lora_weights_name_or_path is None:
        logging.warning(
            "You have specified to use LoRA, but have not specified a path to the "
    ...
    if model_args.merge_lora_before_inference:
        ...
        lora_weights_name_or_path = None
        clean_cache()  # Ensure that nothing remains in the cache, as we will load the merged model next.
...
if __name__ == "__main__":
    ...
    # argparser for user input for indicating model
    ...
    argParser.add_argument(
        ...
        type=int, 
        default=0,
        help="Number of examples to add to the prompt"
    )

    custom_args = argParser.parse_args()
    
    if custom_args.config_file.endswith(".json"):
        # If we pass only one argument to the script, and it's the path to a json file,
        # let's parse it to get our arguments.
        logging.info(f"Loading json config {custom_args.config_file}")
        model_args, data_args, training_args = parser.parse_json_file(json_file=os.path.abspath(custom_args.config_file))

    elif custom_args.config_file.endswith(".yaml"):
        # If we pass only one argument to the script, and it's the path to a yaml file,
        # let's parse it to get our arguments.
        logging.info(f"Loading yaml config {custom_args.config_file}")
        model_args, data_args, training_args = parser.parse_yaml_file(yaml_file=os.path.abspath(custom_args.config_file))
    else:
